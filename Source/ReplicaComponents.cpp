...
void DestructibleComponent::writeToPacket(RakNet::BitStream *packet, REPLICA_PACKET_TYPE packetType){
	...
	if (flag4){
		...
		packet->Write(t); //long, count for following structures
		for (int i = 0; i < t; i++)
		{
			packet->Write((long) data4_1.at(i));
		}
		

		packet->Write(trigger); //boolean true

		if (packetType == REPLICA_CONSTRUCTION_PACKET){

			packet->Write(data4_2); //boolean //false

			packet->Write(data4_3); //boolean //false

			if (trigger){
				packet->Write(false); //boolean //false

				packet->Write(flag4_4_2); //boolean //false

				if (flag4_4_2){
					packet->Write(data4_4_2); //long
				}
			}
		}
		//flag4 = false;
